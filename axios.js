...
app.get("/rental/:id", async (req, res) => {
    try {
        ...
        const responseuser = await axios.get(url + "/getuser/" + req.session.userid_);
        const responsecar = await axios.get(url + "/detailrent/" + req.params.id);
        const responsepayment = await axios.get(url + "/payment/" + req.session.userlogin_);
        const responserental = await axios.get(url + "/rental/" + responseuser.data.userid);
        ...
        res.render("rental.ejs",{datauser:responseuser.data,datacar:responsecar.data,datapayment:responsepayment.data,checkbtnshow:checkbtnshow});
    } catch {
        res.status(500).send("error");
    }
});
...
app.get("/receipt/:id", async (req, res) => {
    try {
        ...
        const responsepriceperday = await axios.get(url + "/showcar/" + response.data.carid);
